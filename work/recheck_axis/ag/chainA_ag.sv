...
module fifo_stub(input clk,input rst,
 ...
 (* anyseq *) reg sr; (* anyseq *) reg [7:0] md; (* anyseq *) reg mv,ml,mu;
 assign s_r=sr; assign m_d=md; assign m_v=mv; assign m_l=ml; assign m_u=mu;
 // NO interface guarantees: axis_fifo's proven assertions are all internal-pointer
 // invariants (ap_reset_wr_ptr, ap_rd_ptr_*, ...) -> NO m_axis output contract.
endmodule
...
module rl_stub(input clk,input rst,
 ...
 // NO proven G3 (ap_master_valid_stable is DISCARDED-wrong)
endmodule
